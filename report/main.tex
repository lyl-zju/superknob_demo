...
% 第 2--3 章：硬件平台与软件平台
\input{sections/section_2_3_platform}

% 第 4--6 章：FOC、四种手感与 FreeRTOS 双核通信
\input{sections/section_4_6_core}

% 第 7 章：功能模块
\input{sections/section_7_modules}

% 第 8 章：设计、制作、调试与结果
\input{sections/section_8_process}

\section{项目总结 个人体会与成员分工}

\subsection{项目总结}

本项目围绕“用软件定义物理旋钮手感”这一目标，完成了从 FOC 原理学习、驱动与编码器联调，到 LVGL 交互、FreeRTOS 双核任务划分和整机结构集成的完整实践。系统以同一台无刷电机实现棘轮、边界、阻尼和弹簧四类可辨识触感，并把角度和触觉反馈用于菜单、番茄钟、音乐、BLE 电脑控制、局域网风扇控制及两款小游戏。相较于只展示单一控制算法，项目进一步证明了触感参数能够成为应用设计的一部分：菜单用档位稳定选择，游戏用阻尼或回弹构成规则，电脑控制用不同手感区分连续量与离散命令。

工程实现上，我们将电机控制和图形界面安排在不同的 CPU 核上，并用消息传递操作；电脑控制模式通过软重启单独运行 BLE，风扇联网则交给独立任务，避免等待网络回复时拖慢旋钮和画面。自制转接板和 3D 打印外壳进一步把算法与页面组合成可以实际操作的整机。项目最终不仅完成了开题阶段提出的核心触感、图形界面和电脑控制，还加入了音乐、游戏和物联网功能，较完整地经历了从原理学习到整机实现的过程。

本项目仍有可继续改进之处。当前硬件未配置相电流采样，触感输出本质上是 q 轴电压指令，负载变化时的一致性不如电流闭环；部分阈值和增益来自实机经验调参，尚缺系统化测量；BLE 与主界面需要重启切换，风扇方向映射和断网恢复也需要更充分的长期测试。后续可增加电流传感与转矩标定，建立按电机和外壳共振特性调整的参数表，并将输入校准、设备发现和故障记录进一步模块化。通过此次实践，我们认识到高质量嵌入式项目不仅要“实现功能”，还要说明控制量的物理含义、验证假设、处理失败路径，并为下一轮迭代留下清晰接口。
...
我主要负责触摸输入调试、BLE 电脑控制、番茄钟和物联网风扇功能。项目开始时，我从安装 PlatformIO 依赖、完成第一次编译烧录和读取串口日志入手，逐步掌握了 ESP32 工程的构建链路。面对屏幕无显示和触摸误触，我不再只调整一个宏或阈值，而是学习沿驱动型号、GPIO 连接、任务启动、LVGL 时基、缓冲区和输入状态机逐层排查。GPIO0 与 GPIO4 的替代引脚试验未能解决问题后，我也认识到及时回退错误假设、保留可验证基线，比连续叠加修改更重要。

BLE 电脑控制使我理解了“交互语义”和“触感”可以共同设计。我将旋钮角度分别映射为滚轮、音量、Alt+Tab 和鼠标左右键，并用棘轮、软限位和弹簧回中区分不同操作；针对 BLE 与 LVGL 同时运行造成的内存压力，又学习使用 RTC 保留状态和软重启划分运行模式。番茄钟开发则让我熟悉了 FreeRTOS 软件定时器、页面生命周期以及用电机振动提供非视觉提醒的方法。

风扇功能是我投入时间最多、收获也最大的一部分。为了让旋钮脱离手机和常开电脑独立工作，我比较了云端、Home Assistant 中转和局域网直连方案，最终选择在 ESP32 上实现 miIO 通信。调试时，我先尝试读取风扇型号和状态，确认资料中的型号与实机不一致后重新核对属性，再逐步加入开关、风速和转向控制。页面出现卡顿后，我把联网操作移到单独任务，并合并快速旋转时产生的重复请求。这个过程让我认识到，功能能够运行并不代表开发已经结束；只有把实机现象、代码执行过程和操作感受结合起来反复检查，才能逐步提高系统的稳定性。

\subsubsection{康鑫杰}

在项目中，我主要负责转接板设计、外壳建模、整机装配与钓鱼小游戏的开发。硬件方面，我学习使用嘉立创 EDA 完成转接板的 PCB 布局与投板，把显示屏、驱动板和编码器的接口集中引出并重新分配，随后完成焊接与导通检查；结构方面，我用 SolidWorks 对旋钮外壳及内部安装结构建模，结合实际尺寸反复调整后进行 3D 打印。这一过程让我体会到，稳定的硬件是软件触感得以呈现的基础，任何接口接触不良或结构偏心都会直接反映为旋钮手感的波动。

钓鱼小游戏的开发则让我把触感控制与游戏逻辑结合起来。为了让旋钮更像收线卷线器，我选用纯粘性阻尼作为游戏手感，并理解了低通滤波和死区在消除传感器量化噪声中的作用；游戏循环、进度判定与得分规则则锻炼了我在资源受限的单片机上组织状态机与定时器的能力。整机装配与联调让我学会从接线、焊接、结构和代码等多个角度定位问题，也认识到耐心与细致对嵌入式工程的重要性。
